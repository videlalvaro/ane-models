#!/usr/bin/env python3
"""Edge Canary Aion-1.0-Instruct ONNX bundle → CoreML / ANE.
...
class OnnxWeightStore:
    """Load ONNX initializers and resolve common Qwen-style names."""
...
def infer_config(bundle: Path) -> Qwen3Config:
...
class Qwen3StatefulLayer:
    def __init__(self, layer_idx: int, weights: OnnxWeightStore, cfg: Qwen3Config, d_ff: int):
        import torch
...
class Qwen3StatefulModel:
    def __init__(self, weights: OnnxWeightStore, cfg: Qwen3Config, max_seq_len: int):
        import torch
        import torch.nn as nn
        ...
        self.layers = [Qwen3StatefulLayer(i, weights, cfg, self.dff) for i in range(cfg.num_hidden_layers)]
...
def _build_coreml_model(bundle: Path, out_dir: Path, max_seq_len: int, quantize_int8: bool, output_kind: str) -> tuple[Path, tuple[int, int]]:
    ...
    model = Qwen3StatefulModel(weights, cfg, max_seq_len).module(output_kind=output_kind)
    model.half().eval()
