...
This script copies tokenizer/config files from the local bundle, rebuilds the
decoder as a stateful CoreML package, and compiles the result for ANE-targeted
execution.

The output directory is local build state. The repository contains the converter
and runtime, not the downloaded model bundle, tokenizer, embedding binary, or
compiled CoreML package.

For fastest correctness validation while iterating, pass `--no-int8` to skip
extra post-conversion quantization. The source bundle's ONNX weights are already
...
| [11 — ONNX Bundles to ANE](book/10-onnx-to-ane.md) | ONNX Runtime contrib ops, local weight materialization, CoreML rebuilds |
